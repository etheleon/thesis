...
#info <- read.delim('out/8015.genome.info.txt', colClass = c('character', 'integer', 'factor', 'character', 'character'))
#gname <- info$info
#names(gname) <- info$id
#gsize <- info$len
#names(gsize) <- info$id
...
#
#files <- list.files('out/8110.map', full = T)
#
...
batch1 <- subset(raw, grepl('^b1-', sample))
...
batch2 <- subset(raw, grepl('^b2-', sample))
...
save(all, batch1, batch2, individual, raw, file = 'out/8120.genome.rda')
